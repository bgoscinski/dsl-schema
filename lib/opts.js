...
function tryParseRangeOpt(str) {
    ...
    if (!lop && !rop)
        return;
    if (lop && rop && lop.charAt(0) !== rop.charAt(0))
        return;
    return Object.assign({ type: 'range', binding }, (lop && getLeftBoundary(lop, parseFloat(lhs))), (rop && getRightBoundary(rop, parseFloat(rhs))));
}
exports.range = binding => opt => opt.type === 'range' && opt.binding === binding;
//                      | lhs   |          | bind |           | rhs   |
...
exports.mult = binding => opt => opt.type === 'mult' && opt.binding === binding;
function parse(spec) {
    if (!spec)
        return [];
    return spec.split(/\s*,\s*/).reduce((acc, part) => {
        const parsed = tryParseUniq(part) || tryParseRangeOpt(part) || tryParseMult(part);
